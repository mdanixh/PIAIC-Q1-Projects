...
let jsonData: Instructor[] = [];

let courseJson: Course[] = [];

let databaseJson: Student[] = [];
...
function readDataforInstructor(filePath: string): any {
    try {
...
function readDataforCourse(filePath: string): any {
    try {
...
function writeDataforInst(filePath: string, data1: any) {
    fs.writeFileSync(filePath, JSON.stringify(data1, null, 400), 'utf-8');
}


function writeDataforCourse(filePath: string, data1: any) {
    fs.writeFileSync(filePath, JSON.stringify(data1, null, 400), 'utf-8');
}
...
class Student extends Person {
    ...
    feeStatus!: boolean;
        ...
        private static studentCounter: number = 1;
        private static rollNoCounter: number = 1;
    
        constructor(id: string, name: string, age: number, feeStatus: boolean) {
            super(id, name, age);
            this.rollNo = this.generateRollNo();
            this.feeStatus = feeStatus;
        }
    
        private generateRollNo(): string {
        ...
        static createStudent(name: string, age: number, feeStatus: boolean): Student {
        ...
        assignCourse(courseName: string) {
...
class Instructor extends Person {
    ...
    salary: number;
        ...
        private static instructorCounter: number = 1;
        private static pinCounter: number = 1;
    ...
    static createInstructor(name: string, age: number, salary: number): Instructor {
        // Generate IDs like DS001, DS002, DS003, ...
...
async function main() {
    while (true) {
        ...
        switch (choice.action) {
            ...
            const newStudent = Student.createStudent(
                inqAddStd.name,
                parseInt(inqAddStd.age),
                inqAddStd.feeStatus
            );
            ...
            const course = dataForCourse.find((course: Course) => course.courseName === courseSelection)
            ...
            case 'Add Instructor':
                ...
                const inqAddIns = await inquirer.prompt([
                    ...
                    const newInstructor = Instructor.createInstructor(inqAddIns.name,
                        parseInt(inqAddIns.age), 
                        parseInt(inqAddIns.salary)); 
            ...
            case 'Add Course':
                ...
                const instructor = readInstructorData.find((instructor: Instructor) => instructor.name === selectedInstructor)
            ...
            case 'Student Details':
                ...
                const readStudentsData = readDataFromFile(studentsFilePath);
                    ...
                    const student = readStudentsData.find((student: Student) => student.id === studentId);
